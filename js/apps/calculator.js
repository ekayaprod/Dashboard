...
function initializePage() {
    ...
    (async () => {
        try {
            ...
            function detectBreakOptimization(currentBreak, totalShiftMinutes, currentTarget) {
                // Break Options: 0, 30, 45, 60, 90, 120
                const options = [0, 30, 45, 60, 90, 120];
                const nextOptions = options.filter(b => b > currentBreak);

                for (const newBreak of nextOptions) {
                    const postBreak = totalShiftMinutes - newBreak;
                    const leeway = postBreak * CONSTANTS.LEEWAY_RATIO;
                    const newProd = postBreak - leeway;
                    const roundedHours = Math.round(newProd / 60);
                    const newTarget = roundedHours * CONSTANTS.TICKETS_PER_HOUR_RATE;

                    if (newTarget < currentTarget) {
                        return {
                            newBreak,
                            savedTickets: currentTarget - newTarget
                        };
                    }
                }
                return null;
            ...
            function calculateDailyRatings() {
                ...
                if (optimizationTip) {
                    ...
                    const currentBreak = parseInt(DOMElements.breakTime.value, 10);
                    const breakOpt = detectBreakOptimization(currentBreak, totalShiftMinutes, targetTicketGoal);
                    if (breakOpt) {
                        tips.push(`<strong>🍱 Lunch Gambit:</strong> Taking a <strong>${breakOpt.newBreak} min</strong> break (instead of ${currentBreak}) drops target by ${breakOpt.savedTickets} tickets.`);
                    }
