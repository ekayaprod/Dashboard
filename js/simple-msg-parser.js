...
(function(global) {
    ...
    SimpleMsgParser.prototype.parse = function(arrayBuffer) {
        ...
        // CRITICAL FIX: If we didn't find at least a subject, body, or recipient,
        // consider it a failure so the full parser can run.
        if (!result.subject && !result.body && !result.to) {
            throw new Error('Simple parser found no meaningful data (subject, body, or to).');
        }
