 ...
 * Version: 2.1.0 (Full Implementation)
 */
...
function parseMailto(mailtoStr) {
    ...
    // Basic validity check, though we proceed gracefully
    if (!/^mailto:[^\s]*/.test(mailtoStr)) return data;

    try {
        // Replace spaces with %20 to help URL constructor
        ...
        // Fallback for simple mailto without query params
...
function buildMailto(data) {
    ...
    MAILTO_FIELDS.slice(1).forEach(field => {
        if (data[field]) {
            if (field === 'body') {
                // Normalize newlines and encode specifically for mail clients
...
function getBreadcrumbPath(folderId) {
    ...
    let iterations = 0;
    const maxIterations = (state.library.length || 1) * 10; // Safety brake

    const stack = state.library.map(item => [item, []]); // [item, parentPath]
...
function toggleAccordion(e, forceCollapse = false) {
    // Prevent toggle if clicking internal buttons within header
    if (e && e.target.closest('button') && e.currentTarget.id === 'mailto-accordion-header') {
        e.stopPropagation();
        return;
    }
    ...
    if (!isCollapsed) {
        // Resize textareas when becoming visible
...
function clearEditorFields() {
    ...
    MAILTO_FIELDS.forEach(field => {
        const id = 'result' + field.charAt(0).toUpperCase() + field.slice(1);
...
function handleTreeItemMove(id, item) {
    ...
    const modalList = document.querySelector('.move-folder-list');
    if (modalList) {
...
function handleFile(file) {
    try {
        ...
        // Check extensions
        const validExts = ['.msg', '.oft', '.eml', '.email'];
        ...
        reader.onload = (e) => {
            try {
                ...
                // Use MsgReader to parse the buffer
                ...
                // Open accordion and resize
                toggleAccordion(null, false);
                setTimeout(() => DOMHelpers.triggerTextareaResize(DOMElements.resultBody), 10);
...
function generateAndShowMailto() {
    ...
    if (!templateNameValue.trim()) {
        const subject = mailtoData.subject || 'New Template';
        DOMElements.saveTemplateName.value = subject.replace(/[\r\n\t\/\\]/g, ' ').trim();
    }
...
function handleSaveToLibrary() {
    ...
    if (!SafeUI.validators.notEmpty(name)) return SafeUI.showValidationError('Invalid Name', 'Template name cannot be empty.', 'save-template-name');
    ...
    clearEditorFields();
    toggleAccordion(null, true); // Collapse after save
...
async function init() {
    ...
    if (typeof MsgReader === 'undefined' || typeof MsgReader.read !== 'function') {
        const errorMsg = "Critical Dependency Missing: MsgReader is not loaded or invalid.";
        console.error(errorMsg);
        const banner = document.getElementById('app-startup-error');
        if (banner) {
            banner.innerHTML = `<strong>Application Startup Error</strong><p>${errorMsg}</p>`;
    ...
    try {
        ...
        if (navContainer && navResponse.ok) {
            ...
            navContainer.innerHTML = `<div style="color:red; padding:1rem;">Error: Could not load navbar.html.</div>`;
        }
    } catch (e) {
    ...
    // Setup UI
    DOMElements.btnNewFolder.innerHTML = SafeUI.SVGIcons.plus + ICONS.folder; // Folder icon
    ...
    if (state.ui) {
        ...
        renderCatalogue();
        initAccordion(); // IMPORTANT: Set initial accordion state
...
function attachEventListeners() {
    ...
    DOMElements.treeListContainer.addEventListener('click', (e) => {
        ...
        if (!itemEl) return;
        if (e.target.closest('.list-item-name')) return; // Allow text selection/click
