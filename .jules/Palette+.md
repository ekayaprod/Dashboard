...
### Calculator (`calculator.html`)\n- **Target:** `.target-card` (The Flat Monolith, The Rigid State)\n- **Injection:** Updated transitions and injected fluid hover depth (`transform: translateY(-2px)`, `box-shadow: var(--box-shadow-lg)`) bounded by a `prefers-reduced-motion` guard.\n\n### Dashboard (`dashboard.html`)\n- **Target:** `.shortcut-item` (The Rigid State)\n- **Injection:** Injected `transition: all 0.2s ease-in-out` base, and fluid hover depth (`transform: translateY(-2px)`, `box-shadow: var(--box-shadow-lg)`) bounded by a `prefers-reduced-motion` guard.\n\n### MailTo (`mailto.html`)\n- **Target:** `.accordion-header` (The Rigid State)\n- **Injection:** Added a missing `:hover` state providing a subtle `background-color: var(--panel-bg)` highlight for interactive feedback.\n

## [$(date +"%Y-%m-%d")] Design Decision Ledger: CTA Premium Enhancements
